...
async function searchGoogleMaps() {
  try {
    ...
    let filename = 'businesses.json';
    let count = 1;

    // Check if the file already exists
    while (fs.existsSync(filename)) {
      filename = `businesses_${count}.json`;
      count++;
    }

    // Write the 'businesses' array to a JSON file
    const businessesJSON = JSON.stringify(businesses, null, 2);
    await fs.writeFileSync(filename, businessesJSON, 'utf8');

    console.log('Businesses data saved to businesses.json');
